...
def test_verify_cached_file_missing_returns_false(tmp_path: Path) -> None:
    manager = CacheManager(cache_dir=tmp_path)
    result = manager.verify_cached_file(str(tmp_path / "missing.wav"), "deadbeef")
    assert result is False


def test_load_metadata_returns_none_for_non_mapping(tmp_path: Path) -> None:
    manager = CacheManager(cache_dir=tmp_path)
    key = "ab"
    metadata_path = manager._metadata_path(key)
    metadata_path.parent.mkdir(parents=True, exist_ok=True)
    metadata_path.write_text(json.dumps(["not", "a", "dict"]), encoding="utf-8")
    assert manager._load_metadata(key) is None
    assert manager.get_cached_path(key) is None
